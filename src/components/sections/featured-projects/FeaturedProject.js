...
import { ExternalLink, CheckCircle2, ChevronUp, ChevronDown, ImageOff } from "lucide-react";
import { Container } from "@/components/ui/container";
import { Button } from "@/components/ui/button";
import { gainioProject } from "@/data/featureProject";
import { featuredContainer, fadeInUp, fadeInLeft, fadeInRight } from "./animations";
...
export function FeaturedProject() {
  ...
  const navigate = (dir) => {
    setDirection(dir);
    setActiveTab((prev) => Math.min(Math.max(prev + dir, 0), gainioProject.highlights.length - 1));
  };
      ...
      <Container>
        ...
        >
          ...
          <motion.div variants={fadeInUp} className="space-y-4">
            <p className="text-sm uppercase tracking-[0.35em] text-[var(--primary)]">Featured Work</p>
            ...
            <div className="grid lg:grid-cols-[1.1fr_0.9fr]">
              ...
              >
                <div className="flex flex-col items-center gap-3 text-[var(--foreground-muted)]">
                  ...
                  <p className="text-xs uppercase tracking-[0.2em] opacity-50">Screenshot coming soon</p>
                </div>
                {/* overlay gradient */}
              ...
              {/* project identity */}
              <motion.div variants={fadeInRight} className="flex flex-col justify-center gap-6 p-8 lg:p-10">
                {/* badges */}
                ...
                <div className="flex flex-wrap gap-3">
                  <a href={gainioProject.liveUrl} target="_blank" rel="noopener noreferrer">
                    <Button size="default" variant="default" className="gap-2 rounded-full">
                ...
                <div className="flex flex-wrap gap-2">
                  {gainioProject.highlights.map((h, i) => (
                    <button
                      ...
                      style={{
                        borderColor:     i === activeTab ? "var(--primary)" : "var(--border)",
                        color:           i === activeTab ? "var(--primary)" : "var(--foreground-muted)",
                        backgroundColor: i === activeTab ? "var(--primary)" + "18" : "transparent",
                      }}
                    >
                      {i === activeTab && (
                        <motion.div
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <AnimatePresence mode="wait" initial={false}>
                      ...
                      >
                        <h4 className="font-semibold text-[var(--foreground)]">{highlight.title}</h4>
                      ...
                      {gainioProject.highlights.map((_, i) => (
                        <span
                          ...
                          style={{
                            ...
                            backgroundColor: i === activeTab ? "var(--primary)" : "var(--border)",
                          }}
                        />
                      ))}
                    </div>
